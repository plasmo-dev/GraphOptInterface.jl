...
function all_neighbors(graph::GraphModel, nodes::Vector{Node})::Vector{Node}
...
function all_neighbors(graph::GraphModel, block::AbstractBlock)::Vector{Node}
	return all_neighbors(optimizer, block.nodes)
end

# neighbors in parent block
function parent_neighbors(graph::GraphModel, block::AbstractBlock)::Vector{Node}
	root = MOI.get(optimizer, BlockStructure())
...
function neighbors(graph::GraphModel, block::AbstractBlock)::Vector{Node}
	root = MOI.get(optimizer, BlockStructure())
	parent_block = root.block_by_index[block.parent_index]
	neighbors = all_neighbors(optimizer, block)
...
function all_incident_edges(graph::GraphModel, block::AbstractBlock)::Vector{Edge}
	root = MOI.get(optimizer, BlockStructure())
...
function parent_incident_edges(graph::GraphModel, block::AbstractBlock)::Vector{Edge}
	root = MOI.get(optimizer, BlockStructure())
	parent_block = root.block_by_index[block.parent_index]
...
function incident_edges(graph::GraphModel, block::AbstractBlock)::Vector{Edge}
	root = MOI.get(optimizer, BlockStructure())
	parent_block = root.block_by_index[block.parent_index]
	inc_edges = all_incident_edges(optimizer, block)
...
function children_incident_edges(graph::GraphModel, block::AbstractBlock)::Vector{Edge}
	children_edges = []
