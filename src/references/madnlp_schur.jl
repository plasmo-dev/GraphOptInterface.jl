...
# Schur solver requires vector of the partitions
...
# Create TSP from block information
function TwoStagePartition(csc, blk_data)
end
...
Instantiate a new Schur linear solver

    `csc`: the linearized KKT system
"""
function Solver(csc::SparseMatrixCSC{Float64};
    ...
    # create the partition of the KKT system
    tsp = TwoStagePartition(csc, opt.schur_part, opt.schur_num_parts)
    # tsp = TwoStagePartition(csc, block_data)

    # first stage primal duals
    ...
    # first stage primal-dual vector
...
function SolverWorker(
    tsp::TwoStagePartition,
    V_0,csc::SparseMatrixCSC{Float64},
    inds::Vector{Int},
    k,
    subproblem_solver::AbstractLinearSolver,
    logger::Logger,
