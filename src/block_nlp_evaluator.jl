...
# we assume that the edge models have been created through the modeling interface
struct EdgeModel <: MOI.ModelLike
...
function MOI.set(model::EdgeModel, ::MOI.NLPBlock, nlp_data::MOI.NLPBlockData)
...
function MOI.eval_objective(model::EdgeModel, x)
...
function MOI.eval_objective_gradient(model::EdgeModel, grad, x)
    if model.sense == MOI.FEASIBILITY_SENSE
...
function MOI.eval_constraint(model::EdgeModel, g, x)
...
function MOI.jacobian_structure(model::EdgeModel)
...
function MOI.eval_constraint_jacobian(model::EdgeModel, values, x)
...
function MOI.hessian_lagrangian_structure(model::EdgeModel)
    H = MOI.hessian_lagrangian_structure(model.qp_data)
    append!(H, MOI.hessian_lagrangian_structure(model.nlp_data.evaluator))
    return H
end

function MOI.eval_hessian_lagrangian(model::EdgeModel, H, x, σ, μ)
